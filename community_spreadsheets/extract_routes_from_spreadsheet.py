...
from ccsr.game.data import get_version_data
from ccsr.routes import RouteAction, RoutePlan, write_route
from ccsr.routes.layout import community_route_type
...
ONLINE_ROUTE_ALGORITHM = "community_route"
...
def _player_profile(click_rate):
    profiles = {
        0.0: "neverclick",
        10.0: "default_10_cps",
        15.0: "default_15_cps",
        200.0: "default_200_cps",
    }
    try:
        return profiles[click_rate]
    except KeyError as error:
        raise ValueError(
            f"No player profile for community click rate: {click_rate:g}"
        ) from error
...
def extract_routes(spreadsheet_directory, output_directory):
    ...
    for workbook in sorted(spreadsheet_directory.glob("*.xlsx")):
        ...
        for row_number, values in _route_rows(workbook):
            ...
            goal = ("one_billion" if category_name == "hardcore" else
                    "one_million" if category_name in {"neverclick", "one_million"}
                    else category_name)
            route_type = community_route_type(goal, click_rate, initial_state)
            destination = output_directory / route_type / f"community_quickster_{_slug(author)}.route"
            source = (
            ...
            plan = RoutePlan(
                ...
                source=source,
                goal=goal,
                route_profile=route_type,
                achievement_curve=(
                    "heavenly_chip_estimate" if category_name == "heavenly_chip" else None
                ),
...
def main(argv=None):
    ...
    parser.add_argument(
        "--source",
        default=REPOSITORY / "community_spreadsheets",
    )
    parser.add_argument(
        "--output",
