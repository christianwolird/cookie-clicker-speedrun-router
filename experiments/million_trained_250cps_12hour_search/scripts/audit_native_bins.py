...
variants=[(f'bins{bins}_quota{quota}',160,400,1000,replace(common,native_anchor_all=4,strategy_bins=bins,diverse_queue=quota,diverse_results=quota,future_upgrade_mask=32768)) for bins in (0,1,2) for quota in (1,2,3)]
rows=[]
...
(work/'audit_native_bins.json').write_text(json.dumps(rows,indent=2)+'\n')
