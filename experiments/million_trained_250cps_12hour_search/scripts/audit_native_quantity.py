...
variants=[(f'quantity{mode}_{pops}',160,400,pops,replace(common,native_anchor_all=4,canonical_partial=True,future_upgrade_mask=32768,quantity_children=mode)) for mode in (1,2,3) for pops in (1000,5000)]
rows=[]
...
(work/'audit_native_quantity.json').write_text(json.dumps(rows,indent=2)+'\n')
